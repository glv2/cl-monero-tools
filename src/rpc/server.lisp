...
(defun server-get-block-transactions (block-id &key (host *rpc-host*) (port *rpc-port*) (user *rpc-user*) (password *rpc-password*))
  (let* ((block-info (server-get-block block-id
                                       :host host
                                       :port port
                                       :user user
                                       :password password))
         (miner-transaction (geta (decode-json-from-string (geta block-info :json)) :miner--tx))
         (transaction-hashes (geta block-info :tx--hashes)))
    (acons "miner-transaction" miner-transaction
           (pairlis transaction-hashes (server-get-transactions transaction-hashes)))))
